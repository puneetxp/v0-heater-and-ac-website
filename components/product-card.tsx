...
export function ProductCard({ product }: ProductCardProps) {
  ...
  const isCooling = product.category.toLowerCase().includes("ac") ||
    ...
    product.category.toLowerCase().includes("split");
    ...
    <Card className="group overflow-hidden hover:shadow-2xl transition-all duration-500 border-0 bg-white dark:bg-slate-900 rounded-xl">
      <CardHeader className="p-0">
        <div className="relative h-64 overflow-hidden bg-muted/30">
          ...
          <Badge
            className={`absolute top-4 right-4 text-white shadow-lg font-semibold ${badgeColor}`}
          >
            {product.capacity}
          </Badge>
        </div>
      ...
      <CardContent className="p-7 space-y-5">
        <div className="space-y-2">
          <div className="text-xs font-semibold text-muted-foreground uppercase tracking-widest opacity-75">
            {product.category}
          </div>
      ...
      <CardFooter className="px-7 pb-7 pt-2 flex gap-3">
        <Button
          asChild
          variant="outline"
          className="flex-1 transition-all duration-300 h-11 font-semibold text-sm border-primary/20 hover:border-primary/40"
          size="lg"
        >
          <Link
            href={detailsUrl}
        ...
        <Button
          asChild
          className="flex-1 bg-primary hover:bg-primary/95 transition-all duration-300 h-11 font-semibold text-sm hover:shadow-lg hover:shadow-primary/25"
          size="lg"
        >
          <Link
            href={`/booking/${product.id}`}
            title={`Securely rent ${product.name} now`}
          >
            Rent Now
          </Link>
        </Button>
